...
if [ ! -f "$UPDATE_DONE" ]; then
	...
	printf "\nUpdate for TheRA RC 1" | tee -a "$LOG_FILE"
	...
	if [ -f "/home/odroid/theraupdate.zip" ]; then
		sudo unzip -X -o /home/odroid/theraupdate.zip -d / | tee -a "$LOG_FILE"
		sudo depmod 4.4.189
		sudo rm -v /home/odroid/theraupdate.zip | tee -a "$LOG_FILE"
	else 
	...
	printf "\nUpdate boot text to reflect current version of TheRA\n" | tee -a "$LOG_FILE"
	sudo sed -i "/title\=/c\title\=*TheRA RC 1 Updated*" /usr/share/plymouth/themes/text.plymouth
